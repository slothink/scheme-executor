 ...
 * - 0% 가동일 경우 0 사용
 */
const REQUIRED_HIT_PERCENT = 0.75;

/**
 * 스크립트 가동율 측정 주기(단위: ms)
 */
const ALIVED_TIMER_INTERVAL = 50;
...
export default class SchemeExecutor {
  ...
  constructor(features: Features, starting: boolean = false) {
    ...
    const appleStandaloneMode = (("standalone" in window.navigator) && window.navigator.standalone);
  ...
  executeSchemeWithFallbackUrl(scheme: Scheme, fallbackUrl?: string, replaceLocation: boolean = false) {
  ...
  executeScheme(scheme: Scheme, fallback?: Fallback, replaceLocation: boolean = false): void {
    ...
    this.debug("ExecuteScheme. scheme:"+JSON.stringify(scheme)+", fallback:"+JSON.stringify(fallback || null));
    ...
    let fallbackTimer = () => {
      ...
      if (diff < 2000) {
        ...
        if (fallback && realHit * 1.0 > requiredHit) { // 유효 타임인 1500 ms 또한 스크립트가 90% 실행되었을 경우의 횟수 = 13
          const fallbackUrl = fallback.fallbackUrl;
          if (fallback.type == 'URL' && fallbackUrl && fallbackUrl.indexOf("http") == 0 && replaceLocation) {
            ...
            if(fallback.type == 'URL') {
              ...
              if (that.features.canNoDelayFallback) {
                if (!fallback.fallbackScheme) {
                  throw Error('[FB-CASE-2-2-1] fallback.type is SCHEME. but fallback.fallbackScheme was undefined');
                }
                let fallbackSchemeExecutor = new SchemeExecutor(that.features, that.starting);
                fallbackSchemeExecutor.executeScheme(fallback.fallbackScheme.scheme, fallback.fallbackScheme.fallback, replaceLocation);
              } else {
                setTimeout(() => {
                  if (!fallback.fallbackScheme) {
                    throw Error('[FB-CASE-2-2-2] fallback.type is SCHEME. but fallback.fallbackScheme was undefined');
                  }
                  let fallbackSchemeExecutor = new SchemeExecutor(that.features, that.starting);
                  fallbackSchemeExecutor.executeScheme(fallback.fallbackScheme.scheme, fallback.fallbackScheme.fallback, replaceLocation);
                }, 1000);
  ...
  fireTimerTrigger(fallbackTimer: Function) {
    this.debug(`Fallback Timer Trigger fired. at: ${new Date().valueOf()}, will trigger after ${this.timerTrigger}`);
